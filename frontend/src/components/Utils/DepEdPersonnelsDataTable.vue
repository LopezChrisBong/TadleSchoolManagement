<template>
  <div>
    ...
    <DepEdPersonnelDialog :data="depedData" :action="action" />
...
import DepEdPersonnelDialog from '../../components/Dialogs/Forms/DepEdPersonnelDialog.vue';
export default {
  components: {
    DepEdPersonnelDialog,
  },
  data: () => ({
    search: '',
    headers: [
  ...
  methods: {
    initialize() {
      this.loading = true;
      this.axiosCall('/user-details/getDepEdPersonnel', 'GET').then((res) => {
        if (res) {
          this.data = res.data;
          this.loading = false;
        }
