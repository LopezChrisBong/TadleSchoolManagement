...
export class RoomsSectionService {
      ...
      for (let i = 0; i < data.length; i++) {
        let saveData = this.dataSource.manager.create(
          StudentQuarterFinalGrade,
          {
            ...
            sub_subject: data[i].MAPEH
              ? JSON.stringify(data[i].MAPEH.sub_subjects)
              : null,
            transmuted_grade: data[i].MAPEH
              ? data[i].MAPEH.transmuted_grade
              : data[i].transmuted_grade,
            initial_grade: data[i].MAPEH
              ? data[i].MAPEH.initial_grade
              : data[i].initial_grade,
            studentID: data[i].studentID,
  ...
  async transmutedGrade(createTransmutedGradeDto: CreateTransmutedGradeDto) {
    ...
    let conflict = await this.conflictTansmuted(
      createTransmutedGradeDto.transmuted_grade,
    );
    if (conflict == 1) {
      return {
        msg: 'Transmuted grade already exist!',
        status: HttpStatus.BAD_REQUEST,
      };
    } else {
      try {
  ...
  async conflictTansmuted(transmuted: number) {
    const genCount = await this.dataSource.query(
      ...
      'SELECT COUNT(*) as count_gen FROM transmuted_grade as t where t.transmuted_grade = "' +
        transmuted +
        '"',
    );
  ...
  async addStudentClassRoom(createAddStudentRoomDto: CreateAddStudentRoomDto) {
    ...
    console.log(
      studentList,
      removeStudent.length,
      createAddStudentRoomDto.classID,
    );
    try {
  ...
  async addMyStudentClassRoom(
    createAddStudentRoomDto: CreateAddStudentRoomDto,
  ) {
    let studentList = JSON.parse(createAddStudentRoomDto.stundent_list);
    let newData = [];
    for (let i = 0; i < studentList.length; i++) {
  ...
  async getCountGen(grade: string, filter: number) {
    const genCount = await this.dataSource.query(
      // 'SELECT COUNT(*) as count_gen FROM student_list where grade_level = "Grade 9" and  school_yearId = "'+filter+'"',
      'SELECT COUNT(*) as count_gen FROM student_list where grade_level = "' +
        grade +
        '" and  school_yearId = "' +
        filter +
        '"',
  ...
  async getMyStudentClassRecords(
    userID: number,
    filter: number,
    roomID: number,
  ) {
      ...
      for (let i = 0; i < grades.length; i++) {
       ...
       LIMIT 1`,
        ...
        if (gradeRecord.length > 0) {
          ...
          grades[i].transmuted_grade = last.transmuted_grade;
        } else {
  ...
  async generateClassRecord(grade: string, filter: number) {
    ...
    try {
      const gradeRecord = await queryRunner.query(
        'SELECT * FROM enroll_student where grade_level = "' +
          grade +
          '" and school_yearId = "' +
          filter +
  ...
  async genStrandRecord(grade: string, filter: number, strandId: number) {
    ...
    try {
      ...
      const room = await queryRunner.query(
        'SELECT *  FROM rooms_section where grade_level = "' +
          grade +
          '" and strandId = "' +
          strandId +
          '"',
  ...
  async AllStrandEnroll(id: number) {
    const toReturn = await this.dataSource.query(
      'SELECT * FROM add_strand where trackId = ' +
        id +
  ...
  updateTransmutedGrade(
    id: number,
    updateTransmutedGradeDto: UpdateTransmutedGradeDto,
  ) {
    try {
  ...
  async updateStudentGrade(
    ...
    try {
      for (const [quizLabel, quizData] of Object.entries(quizzes)) {
        const { id, value, max } = quizData as {
          id: number;
          value: number;
          max: number;
        };
