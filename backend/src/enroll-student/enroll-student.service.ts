...
export class EnrollStudentService {
  ...
  async addSchoolYear(createSchoolYearDto: CreateSchoolYearDto) {
    try {
        ...
        where: { school_year_from: createSchoolYearDto.school_year_from,school_year_to: createSchoolYearDto.school_year_to },
        });
  ...
  async getValuesData(filter:number, studentID:number, roomID:number, quarter:string, semester:string) {
  ...
  async getAllValuesData(filter: number, studentID: number) {
    ...
    const filled = quarters.map(q => {
      return data.find(d => d.semester === q.semester && d.quarter === q.quarter) || {
        studentID,
  ...
  async getAllStudentsFinalGrade(filter:number, roomID:number,quarter:string,semester:string){
...
async getClassProgramm(grade: string , section : number, filter:number ) {
    ...
    // );
      const toReturn = await this.dataSource.query(
      '  SELECT  CONCAT(t1.times_slot_from,  " - ", t1.times_slot_to) AS time ,CONCAT(t2.lname,  " ,", t2.fname) AS name ,t1.id as availId,  t1.*, t2.*, t3.*, t4.* FROM availability t1 LEFT JOIN user_detail t2 ON t1.teacherID = t2.id LEFT JOIN rooms_section t3 ON t1.roomId = t3.id LEFT JOIN subject t4 ON t1.subjectId = t4.id where t1.grade_level = "'+ grade +'" and t1.roomId ="'+section+'" and t1.school_yearId ="'+filter+'" order by field(LOWER(t1.day),"monday", "tuesday", "wednesday", "thursday", "friday", "saturday")' ,      
    );
...
async update_student_file( body:any, filename: any){
  ...
  async updateDropStudent(id:number,updateEnrollStudentDto:UpdateEnrollStudentDto){
  ...
  async updateStudentValues(id:number, updateStudentValuesDto:UpdateStudentValuesDto){
